            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(BACKEND_URL) as response:  # noqa: E501
                    async with in_transaction():
                        ...
                        # filter logged in users only via checking if they have pk in json
                        fetched_ids = [_["pk"] for _ in update_data if _["pk"]]
                        if not fetched_ids:
                            return None
                        # split update data on already existing and new ones
                        existing_users = await User.filter(id__in=fetched_ids)
                        for user in existing_users:
                            user_data = [_ for _ in update_data if _["pk"] == user.id][0]
                            ...
                            user.total_kills = user_data["totalkills"]
                            user.total_deaths = user_data["totaldeaths"]
                            user.total_score = user_data["earnings"]
                        existing_users_ids = [_.id for _ in existing_users]
                        # detect new users
                        new_users_ids = list(set(fetched_ids) - set(existing_users_ids))
                        new_users_data = [_ for _ in update_data if _["pk"] in new_users_ids]
                        new_users = [
                            User(
                                id=_["pk"],
                                name=_["name"],
                                ...
                                total_kills=_["totalkills"],
                                total_deaths=_["totaldeaths"],
                                total_score=_["earnings"],
                            )
